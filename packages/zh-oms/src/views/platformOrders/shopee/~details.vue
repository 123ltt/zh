  ...
  <div class="plat-details">
          ...
          <template slot-scope="{row}">
            <g-list-group label-width="50px" label-align="center">
              <g-list-group-item label="在线SKU">
                <g-text-ellipsis>{{ row.sellerSku }}</g-text-ellipsis>
              </g-list-group-item>
              <g-list-group-item label="商品名称" class="my-1">
                <g-text-ellipsis>{{ row.skuName }}</g-text-ellipsis>
              </g-list-group-item>
    ...
    <el-card class="table-card no-pb">
      ...
      <g-table ref="logTableRef" :dense="true" :headers="logHeaders" :fetch="logFetch" :fixed-height="false">
        <el-table-column slot="msgDetail" label="描述" align="center">
          <template slot-scope="{row}">
            <g-text-ellipsis :lines="2">{{ row.msgDetail }}</g-text-ellipsis>
          </template>
        </el-table-column>
      </g-table>
    </el-card>
...
import { getLogsPage } from '@/api/shopee.js'
...
export default {
  ...
  data() {
    return {
      ...
      goodsHeaders: [
        ...
        { label: '单价', prop: 'discountPrice', formatter: (row, column, cellValue) => this.formatterMoney(cellValue), width: '120px' },
        { label: '数量', prop: 'productCount', width: '120px' },
        { label: '销售金额', prop: 'totalPrice', formatter: ({ discountPrice, productCount }) => this.formatterMoney(discountPrice * productCount), width: '120px' }
      ],
      logHeaders: [
        { label: '序号', type: 'index', align: 'center' },
        { label: '操作属性', prop: 'msgBrief', align: 'center' },
        { slot: true, slotName: 'msgDetail' },
  ...
  mounted() {
    this.$emit('title', 'Shopee订单详情')
    this.$refs.logTableRef.load(1, { platOrderId: this.platOrderId })
  },
  methods: {
    formatterMoney(price) {
      let res = this.$refs.detailsFormRef.form.orderCurrency + ' '
      res += parseFloat(price || 0).toFixed(2)
      return res
    },
    getGoodsInfo(form) {
      form.platformCode = 'Shopee'
      form.ifPreSale = ['', '是', '否'][form.ifPreSale] || ''
      form.ifCashOnOelivery = form.ifCashOnOelivery ? '是' : '否'
