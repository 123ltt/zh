<template>
  <div class="plat-details">
    <wishDetails ref="amazon" :items="itemList" :config="config" />
...
import wishDetails from '../details/details-form.vue'
import { whPlatOrderHis } from '@/api/platformOrder'
...
export default {
  ...
  data() {
    return {
      itemList: itemList,
      config: {
        params: { data: this.platOrderId },
        url: '/whPlatOrderMain/detail',
        methods: 'GET'
      },
      labelWidth: 'auto',
      formData: {},
  ...
  mounted() {
    this.$emit('title', 'wish订单详情')
    this.$refs.table.load(1, { data: this.platOrderId })
  },
  methods: {
    fetch({ current, size }, params) {
      return whPlatOrderHis({ query: { current, size }, ...params }).then(res => {
        return {
