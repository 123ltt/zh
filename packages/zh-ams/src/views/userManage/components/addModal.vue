      ...
      <el-collapse-item name="2">
        ...
        <el-row>
          <el-col :span="19">
            <el-row>
              <el-col :span="12">
                <el-form-item label="主平台" prop="mainPlatformId">
                  <el-select v-model="form.mainPlatformId" placeholder="主平台" filterable clearable>
                    <el-option v-for="plat in platList" :key="plat.hiddenValue" :label="plat.displayName" :value="plat.hiddenValue" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="副平台">
                  <el-select v-model="form.subPlatformId" placeholder="副平台" filterable multiple clearable class="w-100">
                    <el-option v-for="plat in platList" :key="plat.hiddenValue" :label="plat.displayName" :value="plat.hiddenValue" />
                  </el-select>
                </el-form-item>
              </el-col>
        ...
        <el-row v-for="(item, index) in form.userDuties" :key="index">
          <el-col :span="19">
            <el-row>
              <el-col :span="12">
                <el-form-item
...
export default {
  ...
  props: ['current', 'isAdmin', 'orgList', 'postList', 'tenantList','platList'],
  data() {
    ...
    return {
      ...
      rules: {
        ...
        // mainPlatformId: [{ required: true, message: '请选择主平台', trigger: 'change' }]
      },
      // 租户列表
      // tenantList: []
      // 用于提交时判断是否修改
  ...
  created() {
    ...
    if (this.current) {
      ...
      getUserDetail({ id: this.current.id }).then(res => {
        const { account, name, phone, birthday, email, jobNumber, tenantId, sex, status, id, userDuties, mainPlatformId, subPlatformId } = res.data
        this.form = Object.assign(this.form, {
          ...
          id,
          mainPlatformId: mainPlatformId === -1 ? '' : mainPlatformId.toString(),
          subPlatformId: subPlatformId && subPlatformId.split(',')
        })
        ...
        // console.log(this.form.userDuties)
      }).finally(() => {
        this.loading = false
      })
    }
  ...
  methods: {
    ...
    **/
    ...
    submitDialog() {
      this.$refs.dialogForm.validate((valid) => {
        if (valid) {
          ...
          form.subPlatformId = form.subPlatformId ? form.subPlatformId.join() : ''
          submitHandle(form).then((res) => {
            if (res.success) {
    ...
    changeOrg_(value, index, item) {
      ...
      this.form.userDuties[index].isDepPrincipal = false
      // 禁掉已经选择过的公司
      this.orgSelectList.filter(item => {
        if (item.id === value) {
          item.disabled = true
        }
      })
