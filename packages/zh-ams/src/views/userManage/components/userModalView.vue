  ...
  >
    <el-collapse v-model="activeNames">
      ...
      <el-collapse-item name="2">
        ...
        <el-row>
          <el-col :span="11">
            <span class="el-form-item__label">主平台</span>
            <div class="el-form-item__content">{{ current.mainPlatformName || '-' }}</div>
          </el-col>
          <el-col :span="11">
            <span class="el-form-item__label">副平台</span>
            <div class="el-form-item__content">{{ current.subPlatformName || '-' }}</div>
          </el-col>
        </el-row>
        <el-row class="m-b-20">
          <el-col :span="22">
...
export default {
  ...
  created() {
    ...
    getUserDetail({ id: this.row.id }).then(res => {
      ...
      const { mainPlatformId, subPlatformId } = this.current
      const mainPlatformName = this.platList.find(item => item.hiddenValue === mainPlatformId) && this.platList.find(item => item.hiddenValue === mainPlatformId).displayName
      const subPlatformArr = subPlatformId.split(',')
      let subPlatformName = []
      subPlatformArr.forEach(item => {
        this.platList.find(v => v.hiddenValue === item) && subPlatformName.push(this.platList.find(v => v.hiddenValue === item).displayName)
      })
      subPlatformName = subPlatformName.join()
