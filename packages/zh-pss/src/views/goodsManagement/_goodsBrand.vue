<template>
  ...
  <basic-container>
    <g-table ref="table" :headers="headers" row-key="id" :fetch="fetch">
          ...
          <el-row :gutter="10" class="searchBox">
            ...
            <el-col :span="9">
              <el-form-item label="商品类目" prop="categoryId">
                <category-cascader ref="cascader" v-model="submit.categoryId" style="width:80%;" check-strictly :multiple="false" />
              </el-form-item>
            </el-col>
            <el-col :span="9">
              <el-form-item prop="supplierName" label="品牌名称">
                <el-input
                  v-model="submit.goodsBrand"
                  style="width:80%;"
                  clearable
                  placeholder="请输入"
                />
      ...
      <!-- 编码跳转详情 -->
      <el-table-column slot="supplierName" label="供应商名称" min-width="170" align="center" show-overflow-tooltip>
        <template v-slot="{row}">
      ...
      <el-table-column slot="handler" label="操作" fixed="right" align="center" width="150">
        <template slot-scope="scope">
          <el-button
            ...
            type="text"
            @click="enableBtn(scope.row,'enable')"
          >启用</el-button>
          <el-button
            v-if="scope.row.status == 0"
            type="text"
          ...
          <el-button
            v-if="scope.row.status == 0"
            type="text"
            @click="removeBtn(scope.row,'remove')"
          >删除</el-button>
          <el-button
          ...
          <el-button
            v-if="scope.row.status == 1"
            type="text"
            @click="disableBtn(scope.row,'disable')"
          >停用</el-button>
          <el-button
            v-if="scope.row.status != 0"
            type="text"
...
<script>
import { getList, update, remove } from '@/api/goodsManagement/goodsBrand.js'
import { supplier, getMyDictBiz } from '@/api/commonApi/common.js'
import goodsBrandDetail from './components/goodsBrandDetail.vue'
import CategoryCascader from '@/components/category-cascader/index.vue'
import goodsBrandAdd from './components/goodsBrandAdd'
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      getMyDictBiz('pss_product_brand_status').then(res => {
        if (res.code === 200) {
          const data = res.data
    ...
    //   重置
    reset() {
      this.$refs.cascader.checked = []
      this.submit.supplierId = ''
      this.submit.goodsBrand = ''
      this.submit.status = ''
      this.searchData = {}
      this.initialization()
    },
    ...
    addBtn(val) {
      ...
      this.$modal({
        title: (this.isAdd ? '新增商品品牌' : '编辑商品品牌'),
