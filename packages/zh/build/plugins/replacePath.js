...
class ReplacePath {
  rootDir = process.cwd().split(Path.sep).slice(0,-1).join(Path.sep)
  ...
  getSubSys(arr = []) {
    return Object.entries(arr).reduce((prev, [key, value]) => {
      if (/^zh-.+/.test(key) || key === 'global-components') {
        prev.push(key)
      }
      return prev
    }, [])
  ...
  handler(resource) {
    ...
    if(filepath === '@/views/_application.vue') {
      debugger
    }

    // 过滤zh-开头 或 包含node_modules/zh- 的路径
    if (/(^zh-)|([/\\]packages[/\\]zh-\w+)/.test(filepath)) {
      if (Path.isAbsolute(filepath)) {
        filepath = Path.relative(Path.join(process.cwd(), 'packages'), filepath)
      }
