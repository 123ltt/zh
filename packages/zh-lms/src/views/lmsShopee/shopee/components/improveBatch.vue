<template>
  <el-row style="width:90%;margin:0 auto">
    ...
    <div style="padding-bottom: 20px;text-align: right">
      <el-button v-if="['audit','update'].includes(isType)" type="primary" size="medium" @click="handleSave">继续提交</el-button>
  ...
  methods: {
    handleSave() {
      if (this.isType === 'audit') {
        ...
        params.isCheck = true
        wishAudit({ ...params, isCheck: true }).then(res => {
          if (res.code === 200) {
        ...
        wishUpload({ ...this.params }).then(res => {
          if (res.code === 200) {
            this.$message.success(res.msg)
            this.$parent.$emit('close', true)
          }
