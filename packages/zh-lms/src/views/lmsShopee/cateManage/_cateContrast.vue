  ...
  <basic-container class="publish-cate-contrast">
    <el-form ref="dialogForm" size="mini">
      <el-form-item label="站点" prop="site">
        <el-select v-model="site" clearable placeholder="请选择站点" style="width:20%;" @change="getSite">
          <el-option v-for="item in sites" :key="item.displayName" :value="item.displayName" :label="item.displayName" />
        </el-select>
      </el-form-item>
    </el-form>
    ...
    <el-card class="box-card" shadow="never">
      ...
      <span v-for="(value, i) in platformLevels" :key="i">
        ...
        >
          <el-option v-for="item in value" :key="item.id" :value="item.categoryId" :label="item.categoryName" :title="item.categoryName" />
        </el-select>
      </span>
    </el-card>
...
<script>
import { getLevelCategory, saveCategoryContrast, checkCategory } from '@/api/shopee/category'
...
export default {
  ...
  methods: {
    ...
    isContrast(productId) {
      return checkCategory({ site: this.site, productCategoryId: productId }).then(res => res.data)
    },
