<template>
  <div class="store-list">
    <div class="sl-left d-flex flex-column">
      <!-- 添加-@submit.native.prevent 阻止触发刷新页面操作-->
  ...
  computed: {
    currentStoreList() {
      ...
      return list.filter(item => {
        ...
        return r.test(item.name) // || r.test(item.code) 搜索关键字只按name搜索
      })
    },
    platformList2() {
    ...
    mapping: {
      handler(values) {
        ...
        Object.entries(values).forEach(([key, value]) => {
          ...
          // }
          if (value.checkedList.length) result.push({ platformCode: key, storeCode: value.checkedList.map(item => value.list.find(its => its.code === item).name) })
        })
