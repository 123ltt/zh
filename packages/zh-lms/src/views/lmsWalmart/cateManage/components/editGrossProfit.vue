<template>
  <basic-container style="min-width: 0">
    <el-form ref="dialogForm" :model="form" size="mini" label-width="110px" :rules="rules">
      ...
      <!-- <el-form-item label="类目树" prop="path">
        {{ form.path }}
      </el-form-item> -->
      <el-form-item label="平台类目名称" prop="categoryName">
        {{ form.categoryName }}
      </el-form-item>
      <el-form-item label="类目毛利率" prop="grossProfit">
        <el-input v-model="form.grossProfit" v-input.number placeholder="请输入" maxlength="4" show-word-limit />
      </el-form-item>
...
// import { updateGrossProfit } from '@/api/lazada/category'
import { updateGrossProfit } from '@/api/walmart/categroy'
...
export default {
  ...
  data() {
    ...
    return {
      rules: {
        grossProfit: [{ required: true, validator: validateProfit, trigger: 'blur' }]
      },
      form: {},
      oldForm: null
    }
  ...
  methods: {
    ...
    submitDialog(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          // const { id, grossProfit } = this.form
          updateGrossProfit(this.form).then((res) => {
