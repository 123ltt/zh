...
<script>
// import { getCategoryList, getCategoryLog } from '@/api/lazada/category'
import { walmartLog } from '@/api/walmart/categroy'
import { getUserList } from '@/api/amazon/amazon'

export default {
  name: 'LmsWalmartCateManageLogCate',
  data() {
    return {
      userList: [],
      userListCache: [],
  ...
  methods: {
    // 获取类目列表
    // walmartLog() {
    //   return walmartLog({ current: 1, size: 999, ...this.form }).then(res => res.data.records)
    // },
    // 获取用户列表
    getUserList(query) {
    ...
    fetch({ current, size }, params) {
      // this.cateList = await this.walmartLog()
      return walmartLog({ current, size, ...params }).then(res => {
