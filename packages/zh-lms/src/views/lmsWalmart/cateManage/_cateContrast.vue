...
<template>
  <basic-container class="publish-cate-contrast">
    ...
    <el-card class="box-card" shadow="never">
      ...
      <g-select v-model="platformCategoryId" :items="platformList" key-field="id" label-field="categoryName" value-field="id" placeholder="一级类目" @change="getChangeData" />
...
import { saveCategoryContrast, checkCategory, getCategoryList } from '@/api/walmart/categroy'
import { getChildren } from '@/api/pms'

export default {
  name: 'LmsWalmartCateManageCateContrast',
  data() {
    return {
      site: '',
      sites: [],
  ...
  methods: {
    ...
    checkCategory() {
      ...
      // }
      checkCategory({ productCategoryId: productId }).then(res => {
        if (!res.data) return false
        // const data = res.data.reverse()
        // const ids = deepClone(data)
        this.deleteChild(1, this.platformLevels)
        // this.platformCategoryId = []
        this.platformCategoryId = res.data.id // 重新定义为字符串
        if (res.data.id) { // 有对照平台类目
          // const requestArr = []
    ...
    isContrast(productId) {
      return checkCategory({ productCategoryId: productId }).then(res => res.data)
    },
