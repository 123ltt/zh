<template>
  <div>
    <div class="tips">
      ...
      <span class="red">*</span>
      <span>尺寸{{ isSwatch ? '必须是100*100 px' : '在1000*1000 px 以上' }}</span>
    </div>
    <el-row class="m-t-30">
      <el-col :span="3">
    ...
    <div class="upload-img-box">
      <UploadImgModal
        :files="files"
        :img-width="isSwatch ? [100,100] : imgWidth"
        :img-height="isSwatch ? [100,100] : imgHeight"
        :max-size="maxSize"
...
export default {
  ...
  props: {
    ...
    imgDragList: Array // 图片列表所有图
  },
  data() {
    return {
      LIMIT,
      files: [],
      imgType: ['jpg', 'png', 'gif', 'bmp'],
      imgInfo: [
        {
          dataUrl: '',
          imageUrl: '',
      ...
      this.files.filter(file => {
        if (file.response && file.response.code === 200) {
          this.importImgList.push({
            ...
            isPrime: false,
            isExtra: this.imgDragList.filter(its => its.isExtra).length < 8 && !this.isSwatch, // 非swatch图，地址上传默认为副图
            isCheck: !this.imgDragList.some(its => its.isCheck)
          })
        }
      })
    },
    ...
    change(value, index) {
      ...
      if (!url) {
        ...
        this.loadImg(url).then(res => {
          // swatch图
          if (this.isSwatch && (res.width !== 100 || res.height !== 100)) {
            this.$$message.error('图片大小必须是100*100')
            return false
          }
          // 产品图
          if (!res.width || res.width < this.isSwatch ? 100 : [0]) {
            this.$message.error(`图片宽度必须在${this.isSwatch ? 100 : [0]} - ${this.isSwatch ? 100 : [1]}之间`)
            this.imgInfo[index].imageUrl = ''
            return false
          } else if (!res.height || res.height < this.imgHeight[0]) {
            this.$message.error(`图片高度必须在${this.isSwatch ? 100 : [0]} - ${this.imgWidth[1]}之间`)
            this.imgInfo[index].imageUrl = ''
            return false
          }
