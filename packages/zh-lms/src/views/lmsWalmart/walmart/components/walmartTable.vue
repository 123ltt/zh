<template>
  <div>
    <div class="head">
      ...
      <skuCompImport v-if="auditTabActiveName === 'Match Failed'" :check-ids="checkIds" source="walmart" text="sku对照表导入" @refreshList="getListingData()" />
      ...
      <el-button v-if="auditTabActiveName === 'Draft'" size="mini" @click="handlerAllImport">批量导入UPC</el-button>
    </div>
    ...
    >
      ...
      <el-table-column v-loading.fullscreen.lock="loading" type="expand">
        <template slot-scope="{ row }">
          <childTable :ref="`childTable${row.id}`"
                      ...
                      :show-up-online-time="showUpOnlineTime"
      ...
      <el-table-column label="SKU" width="160px" align="left">
        <template slot-scope="{row}">
          <span class="d-block">SKU：<g-link class="text-primary" @click="openSpuSkuDetail(row.productSku,row.name)">{{ row.productSpu }}
            <template v-if="row.listingType === 'Variation'">(子 {{ row.variantNum }}）</template></g-link>
          </span>
      ...
      <el-table-column label="标题" prop="name">
        <template slot-scope="{ row }">
          <template v-if="auditTabActiveNum<=4">
            <g-link lass="text-primary" @click="handleTitleCell(row)">
              <g-text-ellipsis>{{ formatStr(row.name) }}</g-text-ellipsis>
            </g-link>
          </template>
        </template>
      </el-table-column>
      ...
      <el-table-column label="后台刊登" prop="isWalmartUpload" width="70px">
        <template slot-scope="{row}">
          {{ row.isWalmartUpload?'是':'否' }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="60px">
        <template slot-scope="{row}">
          <template v-if="['Draft','Upload Failed','Online','Update Failed','Match Failed'].includes(auditTabActiveName)">
            <el-button type="text" size="mini" class="ms-0" @click="handleEdit(row)">编辑</el-button>
            <el-button v-if="['Draft','Upload Failed'].includes(auditTabActiveName)" type="text" size="mini" class="ms-0" @click="handleDel(row)">删除</el-button>
            <template v-else>
...
<script>
import { walmartlisting, getListVariant, walmartlistingDel, getBatchDetail, walmartUpload, walmartAudit } from '@/api/walmart/walmart.js'
...
import { tabActiveList, formatStr } from '../addListing.handle'
export default {
  name: 'WalmartTable',
  components: {
    childTable,
  ...
  computed: {
    ...
    // 上架时间 5-9
    showUpOnlineTime() {
      return ['Online', 'Updating', 'Update Failed', 'Match Failed', 'Canceling', 'Canceled'].includes(this.auditTabActiveName)
    },
  ...
  methods: {
    ...
    oneCheck(row) {
      ...
      if (row.listingType === 'Variation') {
        ...
        if (this.$refs['childTable' + row.id]) {
          this.$refs['childTable' + row.id].yy(row.isSelect)
        }
      }
      this.checkIds = this.tableData.filter(item => item.isSelect === true).map(acc => acc.id)
      this.checkItems = this.tableData.filter(item => item.isSelect === true)
    ...
    getListVariant() {
      getListVariant(this.amazonListingItem.id).then(res => {
        const data = res.data
        data.forEach(item => {
          if (this.showUpOnlineTime && item.status) {
            item.status = this.tabActiveList[item.status].label
          }
          item.isSelect = false
        ...
        if (this.expands.length) {
          ...
          // 全选
            ...
            this.$refs[`childTable${this.amazonListingItem.id}`].allCheck(true)
          }
        }
      })
    },
    ...
    getAmazonlisting(current, size) {
      ...
      this.loading = true
      walmartlisting({ current, size }, { ...this.searchData, listingStatus: Number(this.auditTabActiveNum) }).then(res => {
        const data = res.data
    ...
    getUpdateAmazonListing() {
      walmartUpload({ ids: this.checkIds, isCheck: false }).then(res => {
        if (res.code === 200) {
    ...
    handleTitleCell(item) {
      ...
      this.$newTab({
        path: item.listingType === 'Variation' ? '/lms/walmart/view/varylisting' + item.id : '/lms/walmart/view/listing' + item.id,
    ...
    batchDetails() {
      ...
      getBatchDetail({ walmartListingIds: this.checkIds, walmartVariantIds: amazonVariantIds }).then(res => {
        const data = res.data
        data.forEach((item, index) => {
          this.$newPage({
            path: 'lms/lmsWalmart/walmart/walmartDetail',
            title: '查看Listing',
            data: {
              id: item.id,
    ...
    createListing(row) {
      this.$newTab({
        path: row ? '/lms/walmart/edit/listing' + row.id : '/lms/walmart/add/listing',
    ...
    handleVariationDel(row) {
      ...
      this.$confirm('确定删除?', '提示', {
        ...
        if (item.length) {
          ...
          this.amazonListDel([row.id], '', row.listingStatus)
        }
      })
    },
    // 批量del
    batchDel() {
      this.$confirm('确定删除?', '提示', {
        ...
        const list = this.tableData.filter(item => item.isSelect).map(acc => acc.id)
    ...
    amazonListDel(list, variantIds, typeStatus) {
      walmartlistingDel({ listingId: list, listingStatus: this.auditTabActiveNum, variantId: variantIds || [] }).then(res => {
        this.$message.success(res.msg)
