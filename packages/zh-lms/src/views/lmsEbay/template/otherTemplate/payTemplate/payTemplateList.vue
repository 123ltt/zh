        ...
        <el-row :gutter="10">
          ...
          <el-col :span="7">
            <el-form-item label="Paypal">
              <g-select v-model="form['policyInfo.paymentEmailAddress']" :items="paypalList" key-field="id" label-field="paypalAddress" value-field="paypalAddress" class="w-100" />
            </el-form-item>
          </el-col>
          <el-col :span="7">
...
<script>
import { getPolicyPage, updatePolicyStatus, getPaypalList } from '@/api/ebay/template.js'
...
export default {
  ...
  methods: {
    ...
    getPaypalList() {
      getPaypalList({ current: 1, size: -1 }).then(res => {
        this.paypalList = res.data.records
        this.paypalListCache = [...res.data.records]
      })
    },
