  ...
  <basic-container class="publish-cate-contrast">
    <el-form ref="dialogForm" size="mini">
      <el-form-item label="站点" prop="site">
        <el-select v-model="site" clearable placeholder="请选择站点" style="width:20%;" @change="getSite">
    ...
    <el-card class="box-card" shadow="never">
      ...
      <span v-for="(value, i) in platformLevels" :key="i">
        ...
        >
          <el-option v-for="item in value" :key="item.platCategoryId" :value="item.platCategoryId" :label="item.platCategoryName" :title="item.platCategoryName" />
        </el-select>
      </span>
    </el-card>
...
<script>
import { getLevelCategory, saveCategoryContrast, checkCategory } from '@/api/ebay/category'
...
export default {
  ...
  methods: {
    ...
    getSite() {
      getLevelCategory({ site: this.site }).then(res => {
        ...
        if (data.length) { // 有对照平台类目
          ...
          if (this.platformLevels[0] && this.platformLevels[0].length) { // 平台类目有数据
            ...
            for (let i = 0; i < data.length; i++) {
              requestArr.push(this.getPlatformChild(data[i].platCategoryId))
            }
          }
          Promise.all(requestArr).then((result) => {
            ...
            const platformCategoryId = ids.map(item => item.platCategoryId)
    ...
    getPlatformChild(id) {
      ...
      return getLevelCategory(params).then(res => res.data)
    },
    setPlatLevel(id) {
      let currentLevel
      this.platformLevels.forEach(item => {
        item.forEach(e => {
          if (e.platCategoryId === id) {
    ...
    isContrast(productId) {
      return checkCategory({ site: this.site, categoryId: productId }).then(res => res.data)
    },
    ...
    handleAjax(platformId, productId) {
      const platformKey = this.platformLevels[this.platformLevels.length - 1].find(item => item.platCategoryId === platformId).id
      const params = {
        site: this.site,
