      ...
      this.$modal({
        ...
        callback: (imageUrls) => {
          if (imageUrls) {
            ...
            const changeExtraNum = this.imgDragList.filter(item => item.isExtra)
            const setExtraNum = this.extraImgNum - changeExtraNum.length
            const newExraImgList = []
            imageUrls.forEach((item, i) => {
              if (i < setExtraNum) {
                item.isExtra = true
                newExraImgList.push(item.imageUrl)
              }
            })
            this.$emit('update:extraImagList', [...this.extraImagList, ...newExraImgList])
