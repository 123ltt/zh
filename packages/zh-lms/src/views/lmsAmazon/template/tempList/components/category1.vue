  ...
  <basic-container style="min-width: 0">
    <el-form ref="formRef" :model="form" size="mini" label-width="80px">
      ...
      <el-form-item label="平台类目" prop="platformLevels">
        <div v-for="(citem,index) in form.categoryIdList" :key="index">
          <span v-for="(value, i) in citem.platformLevels" :key="i" class="category-box">
            <el-select
              v-model="citem.platformCategoryId[i]"
              size="mini"
              filterable
              :placeholder="citem.platformRanks[i] + '级类目'"
          ...
          <el-button v-if="index===0" icon="el-icon-plus" />
          <el-button v-if="index!==0" icon="el-icon-minus" />
        </div>
      </el-form-item>
