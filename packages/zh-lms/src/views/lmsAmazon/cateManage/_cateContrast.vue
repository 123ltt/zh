  ...
  <basic-container class="publish-cate-contrast">
    <el-form ref="dialogForm" size="mini">
      <el-form-item label="站点" prop="amazonSite">
        <el-select v-model="amazonSite" clearable placeholder="请选择站点" style="width:20%;" @change="getSite">
          <el-option v-for="(item, ind) in sites" :key="ind" :value="item.displayName" :label="item.displayName" />
        </el-select>
      </el-form-item>
    </el-form>
...
<script>
import { getLevelCategory, saveCategoryContrast, checkCategory } from '@/api/amazon/category'
...
export default {
  ...
  methods: {
    ...
    getSite() {
      getLevelCategory({ amazonSite: this.amazonSite }).then(res => {
        ...
        if (data.length) { // 有对照平台类目
          ...
          if (this.platformLevels[0] && this.platformLevels[0].length) { // 平台类目有数据
            ...
            for (let i = 0; i < data.length; i++) {
              requestArr.push(this.getPlatformChild(data[i].id))
            }
          }
          Promise.all(requestArr).then((result) => {
            ...
            const platformCategoryId = ids.map(item => item.id)
    ...
    isContrast(productId) {
      return checkCategory({ amazonSite: this.amazonSite, productCategoryId: productId }).then(res => res.data)
    },
