...
<script>
import { quantityTemplateDownload, batchUpdateByFile } from '@/api/amazon/amazon'
...
export default {
  ...
  methods: {
    importExc() {
      console.log('importFormObj', this.importFormObj)
    ...
    // 导出
    handlerDown() {
      const name = '导入更新指定价/指定库存.xlsx'
      quantityTemplateDownload({ amazonListingIds: this.selectedIds, variantIds: this.variantIds }).then(res => {
        if (res.size) {
          dowloadExc(res, name)
        }
      })
