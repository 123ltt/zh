<template>
  <div class="shopee-product-image">
    <div class="tips">
      ...
      <span class="red">*</span>大小不能超过2M,
      <span class="red">*</span>尺寸建议1024*1024
    </div>
    <el-row class="m-t-30">
      <el-col :span="3">
...
export default {
  ...
  methods: {
    ...
    updateFiles(val) {
      ...
      if (this.getFiles) this.getFiles(this.files)
    },
    loadImg(url) {
        ...
        this.loadImg(url).then(res => {
          ...
          uploadUrlImg({ url }).then(res => {
            if (res.success) {
              ...
              if (this.getFiles) this.getFiles([{ imageUrl: res.data }])
            } else {
