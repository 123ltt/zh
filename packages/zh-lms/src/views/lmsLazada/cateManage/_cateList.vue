  ...
  <basic-container>
    <g-table ref="table" :headers="headers" row-key="id" :fetch="fetch">
      <template slot="header">
        <el-form ref="form" :model="form" size="mini" label-width="90px" class="no-message form-col" @submit.native.prevent="search">
          <el-row :gutter="10">
            ...
            <el-col :span="5">
              <el-form-item label="平台类目名称" prop="name">
            ...
            <el-col :span="9">
              <el-form-item>
                ...
                <el-button v-if="permission.categoryLog" type="primary" size="mini" @click="showLog">日志</el-button>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
      ...
      <el-table-column slot="handler" label="操作" width="150px">
        <template v-slot="{row}">
          <el-button v-if="permission.setProfit" type="text" size="mini" @click="setGrossProfit(row)">设置销售利润率</el-button>
        </template>
      </el-table-column>
    </g-table>
  </basic-container>
...
import { getCategoryList } from '@/api/lazada/category'
