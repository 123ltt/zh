<template>
  <div>
    <div class="tips">
      ...
      <span class="red">*</span>尺寸在500x500 到 2000x2000 px之间
    </div>
    <el-row class="m-t-30">
      <el-col :span="3">
      ...
      this.files.filter(file => {
        if (file.response && file.response.code === 200) {
          this.importImgList.push({
            ...
            isPrime: false,
            isExtra: this.imgDragList.filter(its => its.isExtra === true).length < this.extraImgNum// 上传图片默认选择副图
          })
        }
      })
    },
        ...
        this.loadImg(url).then(res => {
          ...
          uploadUrlImg({ url }).then(res => {
            if (res.success) {
              ...
              this.imgInfo[index].isExtra = this.imgDragList.filter(its => its.isExtra === true).length < this.extraImgNum // 地址上传图片默认为副图
            } else {
