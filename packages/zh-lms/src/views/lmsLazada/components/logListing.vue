...
<script>
import { listingBusLog, getConstants } from '@/api/lazada/lazadaList.js'
import { getUserList } from '@/api/amazon/amazon'
export default {
  name: 'LmsLazadaListLogListing',
  props: {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { label: '操作类型', prop: 'optType' },
        { slot: true, slotName: 'content' },
        { label: '执行状态', prop: 'optStatus' },
        { slot: true, slotName: 'errorMsg' },
        { label: '操作人', prop: 'optUser', width: 120 },
        { label: '操作时间', prop: 'createTime', width: 140 }
      ],
      form: {
        listingId: ''
      },
  ...
  mounted() {
    this.$set(this.form, 'listingId', this.listingId)
  ...
  methods: {
    getConstants() {
      getConstants().then(res => {
        ...
        this.operationTypes = Object.values(listingOptType).reverse()
    ...
    fetch({ current, size }, params) {
      return listingBusLog({ current, size, ...params }).then(res => {
        const data = res.data.records
        data.forEach((value, j) => {
          value.optContent = value.optContent || ''
          value.optContentDetail = value.optContentDetail || ''
          value.optContent = value.optContent + value.optContentDetail
          if (value.content) {
