...
import { getList, OoCStatus, deleData, getApiList } from '@/api/forecast/apiConfiguration'
  ...
  methods: {
    getData() {
      Promise.all([
        ...
        this.apiList = this.showApiList = res[3].data
      })
    },
    changeChannel(val) {
    ...
    apiEdit(rows) {
      ...
      const apiList = Object.assign([], this.apiList)
      this.$modal({
