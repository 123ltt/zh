      ...
      <template slot="header">
        <el-form ref="form" :model="searchData" label-width="90px" size="mini" class="no-message form-col">
          <el-row :gutter="10">
            ...
            <el-col :span="6">
              <el-form-item label="租户：" prop="tenantId">
                <el-select v-model="searchData.tenantId" clearable placeholder="请选择" filterable class="w-100">
          ...
          <el-row :gutter="10">
            ...
            <el-col :span="6" class=" pb-2  ">
              <el-button class="ms-5" type="primary" icon="el-icon-search" size="mini" @click="search">搜索</el-button>
...
<script>
import { getList, exportLgList } from '@/api/forecast/logisticsSheet'
import {
  logisticsChannel,
...
export default {
  ...
  methods: {
    ...
    getTime() {
      ...
      const m = (date.getMonth() + 1).length > 1 ? (date.getMonth() + 1) : '0' + (date.getMonth() + 1)
      const lM = date.getMonth().length > 1 ? date.getMonth() : '0' + date.getMonth()
      const d = date.getDate()
      this.searchData.createTime[0] = y + '-' + lM + '-' + d + ' 00:00:00'
    ...
    init() {
      const params = JSON.parse(
        JSON.stringify(this.searchData,
          ['channelCode', 'searchType', 'tenantId', 'platformCode', 'type', 'searchValue'])
      )
    ...
    fetch({ current, size }, params) {
      return getList(current, size, params).then(res => {
        ...
        records.forEach(item => {
          ...
          // 转换一下状态
          if (item.status === 30) {
            item.statusValue = this.echoStatusObj[10]
          } else if (item.status === 40) {
            item.statusValue = this.echoStatusObj[20]
          } else {
            item.statusValue = this.echoStatusObj[item.status]
          }

          // 转换物流商名称
          this.logisticsProviderList.forEach(e => {
            if (e.supplierNumber === item.providerCode) item.providerName = e.lpAbbreviation
          })
        })
        return {
          records,
    ...
    downBtn() {
      this.$confirm('确认导出订单查询列表?', {
        ...
        exportLgList(this.searchParams).finally(() => {
          this.loading = false
        }).then(res => {
          dowloadExc(res, '预报订单.xlsx')
