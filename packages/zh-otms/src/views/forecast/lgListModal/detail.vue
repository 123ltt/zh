    ...
    <el-row type="flex" :gutter="8" class="row">
      <el-col v-for="(items, index) in detailRow1" :key="index" :span="12">
        <el-card shadow="hover">
          ...
          <template v-if="index===0">
            <g-list-group v-for="(item,i) in items" :key="i" :cols="2" label-width="60px">
          ...
          <template v-else>
            <el-table
              ...
              stripe
              default-expand-all
              class="g-table g-table-dense ftable"
            >
    ...
    <el-row type="flex" :gutter="8" class="row">
      <el-col v-for="(items, index) in detailRow2" :key="index" :span="12">
        <el-card shadow="hover">
          ...
          <g-list-group v-for="(item,i) in items" :key="i" :cols="2" :label-width="index===0?'60px':'80px'">
            <g-list-group-item v-for="(e,j) in item" :key="j" :label="e.label">
    ...
    <el-tabs v-model="active" lazy>
      <el-tab-pane label="状态" name="showStatus">
        <el-steps :active="statusActive" finish-status="finish" align-center process-status="process" class="mt-4">
          ...
          <el-step v-if="progressResults===2" title="取消中" status="error" :description="logisticsListInfo.finishTime" />
          <el-step v-if="progressResults===3" title="订单取消" status="error" :description="logisticsListInfo.finishTime" />

          <el-step v-if="progressResults===4" title="预报异常" status="error" :description="logisticsListInfo.finishTime" />
        </el-steps>
      </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      // 处理步骤条
      switch (this.logisticsListInfo.status) {
        case 10:
          this.statusActive = 1
          break
        case 20:
