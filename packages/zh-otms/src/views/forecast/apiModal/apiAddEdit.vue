    ...
    <el-form ref="table" label-width="100px" size="mini" :rules="rules" :model="formData">
      ...
      <el-row>
        <el-form-item label="策略名：" prop="strategyName">
          <g-select
            ...
            :items="showApiList"
            key-field="strategyName"
            :label-field="getName"
            value-field="strategyName"
            placeholder="请选择"
            :item-title="true"
      ...
      <el-row>
        <div div class="float-end me-1">
          <el-button size="mini" @click="cancel(false)">取消</el-button>
...
<script>
import { addData, upData } from '@/api/forecast/apiConfiguration'
export default {
  props: {
    isAdd: Boolean,
    rows: Object,
  ...
  computed: {
    ...
    getstrategyName() {
      const item = this.apiList.find(i => i.strategyName === this.rows.strategyName)
      return item ? `${item.strategyName} (${item.strategyName})` : ''
    }
  },
  created() {
    if (this.rows) {
      this.formData.strategyName = this.rows.strategyName
      this.formData.paramJson = this.rows.paramJson
    }
  ...
  methods: {
    ...
    changeProvider(val) {
      if (!this.formData.lpId && val.channelCode) {
    ...
    changeStrategy(val) {
      this.apiList.forEach(item => {
        if (val.data.strategyName === item.strategyName) {
          this.formData.paramJson = item.paramJson
        }
      })
    },
    getLabel(item) {
    ...
    getName(item) {
      return `${item.strategyDesc} (${item.strategyName})`
    },
    cancel(flag) {
