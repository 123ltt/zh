      ...
      <el-col v-for="(item,index) in formItem" :key="item.prop" :span="24/cols">
        ...
        >
          ...
          <template v-if="formItem.length===index+1&&item.type!=='slot'">
            <el-button v-for="itemHandle in formHandler" :key="itemHandle.label" :type="itemHandle.type||'primary'" @click="itemHandle.handler&&itemHandle.handler()">{{ itemHandle.label }}</el-button>
          </template>
        </el-form-item>
        <!-- 垂直列表按钮 -->
