        ...
        <el-row :gutter="4">
          ...
          <el-col :span="16">
            <el-form-item class="dict-button">
              ...
              <el-button size="small" @click="$refs.form.resetFields();form={};form.parentId=parentId;onSearch()">清 空</el-button>
...
import { getDictChildList, removeDict } from '@/api/dict'
...
export default {
  ...
  methods: {
    ...
    getPageData() {
      getDictChildList(this.form).then(res => {
        this.loading = true
        const records = res.data
    ...
    editRow(row) {
      if (!row.id && !this.form.parentId) { // 添加子级
        this.$message.warning('请先选择父级')
        return
      }
      this.$modal({
        ...
        data: {
          ...
          selectedChildData: row.id ? undefined : this.selectedChildData // 子级添加
        },
        callback: (refresh) => {
