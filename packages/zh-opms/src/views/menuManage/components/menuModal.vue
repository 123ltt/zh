    ...
    <el-row :gutter="gutter" class="m-t-20">
      ...
      <el-col :span="12">
        <el-form-item label="菜单排序:" prop="sort">
          <el-input v-model="form.sort" placeholder="请输入菜单排序" @input="changeSort" />
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item label="菜单标签:" prop="label">
          <el-input v-model="form.label" placeholder="请输入菜单标签" />
        </el-form-item>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  data() {
    const validateLabel = (rule, value, callback) => {
      if (value && /^[a-zA-Z][a-zA-Z_0-9]{1,}$/.test(value)) {
        callback()
      } else {
        callback(new Error('请输入标签格式为字母,数字,下划线，以字母开头'))
      }
    }
    return {
