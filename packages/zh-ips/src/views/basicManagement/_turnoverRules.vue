<template>
  ...
  <basic-container>
    <g-table ref="table" :headers="headers" :fetch="fetch" row-key="id">
      ...
      <!-- 状态 -->
      <el-table-column slot="status" label="状态" width="70" align="center">
        <template v-slot="{row}">
...
<script>
import { getList, update, remove } from '@/api/basicManagement/turnoverRules.js'
import { dictionary } from '@/api/commonApi.js'
import turnoverRulesAdd from './components/turnoverRulesAdd'
import turnoverRulesDetail from './components/turnoverRulesDetail'
export default {
  ...
  methods: {
    ...
    addBtn(val) {
      ...
      this.$modal({
        title: (this.isAdd ? '新增周转规则' : '编辑周转规则'),
        component: turnoverRulesAdd,
        width: '30%',
        data: {
          isAdd: this.isAdd,
