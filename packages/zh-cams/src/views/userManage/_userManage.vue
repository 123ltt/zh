  ...
  <el-row>
    <el-col :span="5">
      <basic-container style="min-width: 0">
        <el-input v-model="filterText" size="small" class="filterText" placeholder="输入关键字进行过滤" />
    ...
    <el-col :span="19">
      <basic-container>
        <el-form ref="screenForm" :model="form" size="small" label-width="60px">
          <el-row>
            <el-col :span="5">
            ...
            <el-col :span="6" class="tc">
              <el-button type="primary" size="small" @click=" form.current = 1; loadList(); ">搜 索</el-button>
              <el-button size="small" @click=" $refs['screenForm'].resetFields(); form.current = 1; loadList(); ">清 空</el-button>
            </el-col>
          </el-row>
          <el-row>
            <el-col class="el-col-btn">
              <el-button v-if="permission.user_add" type="primary" size="small" @click="addUser(null)">添加用户</el-button>
              <el-button v-if="permission.resetPassword" size="small" @click="resetPassword">密码重置</el-button>
            </el-col>
          </el-row>
        </el-form>
        <el-table
        ...
        >
          ...
          <el-table-column label="角色" prop="role" min-width="150">
            <template slot-scope="{ row }">
              <span v-for="(item, index) in row.roles" :key="index">
                <el-tag class="tag">{{ item.roleName }}</el-tag>
              </span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 获取岗位列表
    getPostAllList() {
      Promise.all([getPostAllList({ current: 1, size: 999 }), getRoleList({ size: 9999 }), getOrgList({ parentId: this.userInfo.tenant_id })]).then(res => {
        this.postList = res[0].data.records
        this.roleList = res[1].data.records
        this.orgList = res[2].data
        this.treeData = deepClone(this.orgList)
      }).finally(() => {
        this.flag = true
      })
      // this.postList = await getPostAllList({ current: 1, size: 999 }).then(res => res.data.records)
    },
    // 获取左侧组织机构列表
    async getOrgList() {
