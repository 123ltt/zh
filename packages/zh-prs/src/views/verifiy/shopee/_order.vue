...
<script>
import { verifyOrderPage, batchVerifyOrder, getOrderStatus, getStoreCodeList } from '@/api/common'
...
export default {
  name: 'PrsVerifiyShopeeOrder', // 账单对账下级明细页面
  ...
  methods: {
    ...
    showDetail(row) {
      this.$newTab({
        path: '/shopee/bill/orderOff/detail',
        title: '订单核销详情',
        component: detail,
        data: {
          info: { id: row.id, isVerify: 0, platformCode: 'SP' }
        },
        clearCache: true
      })
    },
    ...
    fetch({ current, size }, params) {
      Object.assign(params, { platformCode: 'SP' })
