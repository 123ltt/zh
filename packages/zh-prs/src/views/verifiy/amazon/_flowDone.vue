...
<script>
import { getFlowDonePage, getStoreCodeList, getpayeeTypes } from '@/api/common'
...
export default {
  ...
  data() {
    return {
      ...
      formItem: [
        ...
        { type: 'input', prop: 'billId', label: '账单ID', placeholder: '多个用英文逗号隔开' },
        {
          type: 'select',
          prop: 'payeeType',
          label: '收款方式',
  ...
  methods: {
    ...
    showDetail(row) {
      this.$newTab({
        path: '/amazon/flowDone/detail',
        title: '流水核销详情',
        component: detailBill,
        clearCache: true,
        data: {
          info: { id: row.id, isVerify: 1, platformCode: 'AM' },
          detailType: 'flow'
        }
      })
    },
