<template>
  <basic-container>
    ...
    >
      ...
      <el-table-column slot="operation" width="150" label="账单明细">
        <template v-slot="{row}">
...
export default {
  name: 'PrsVerifiyEbayBillDone', // 已关闭订单
  components: { colsForm },
  data() {
    return {
      headers: [
        { label: 'PayPal账号/店铺编码', prop: 'storeCode' },
  ...
  methods: {
    ...
    showDetail(row) {
      this.$newTab({
        path: '/ebay/flowDone/billDoneDetail',
        title: '账单明细',
