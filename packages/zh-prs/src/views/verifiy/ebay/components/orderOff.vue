...
<script>
import { verifyOrderPage, batchVerifyOrder, getOrderStatus, getStoreCodeList, getSiteNameList } from '@/api/common'
...
import billAdjust from '@/views/verifiy/modal/billAdjust'
...
export default {
    ...
    return {
      ...
      headers: [
        ...
        { label: '站点', prop: 'site' },
        {
          slot: true,
          slotName: 'bill',
          list: [{ label: '平台订单号', prop: 'platformOrderCode' },
  ...
  methods: {
    ...
    getData() {
      ...
      getSiteNameList({ platformCode: 'EB' }).then(res => {
    ...
    showDetail(row) {
      this.$newTab({
        path: '/ebay/bill/orderOff/detail',
        title: '订单核销详情',
        component: detail,
        data: {
          info: { id: row.id, isVerify: 0, platformCode: 'EB' }
        },
        clearCache: true
      })
    },
    ...
    fetch({ current, size }, params) {
      Object.assign(params, { platformCode: 'EB', verifyBillId: this.verifyBillId })
