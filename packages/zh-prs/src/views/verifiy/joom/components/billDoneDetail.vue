...
<script>
import { verifyOrderDonePage, getVerifyType, getStoreCodeList } from '@/api/common'
...
export default {
  ...
  methods: {
    ...
    showDetail(row) {
      this.$newTab({
        path: '/joom/flow/billDoneDetail/detail',
        title: '订单详情',
        component: detail,
        clearCache: true,
        data: {
          info: { id: row.id, isVerify: 1, platformCode: 'JM' }
    ...
    fetch({ current, size }, params) {
      Object.assign(params, {
        platformCode: 'JM', verifyBillId: this.verifyBillId
      })
