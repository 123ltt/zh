...
<script>
import { getFlowPage, getpayeeTypes, getStoreCodeList, getFlowStatus } from '@/api/common'
...
export default {
  ...
  methods: {
    ...
    showDetail(row) {
      this.$newTab({
        path: '/wish/flow/detail',
        title: '流水核销详情',
        component: detailBill,
        clearCache: true,
        data: {
          info: { id: row.id, isVerify: 0, platformCode: 'WH' },
          detailType: 'flow'
        }
      })
    },
    ...
    fetch({ current, size }, params) {
      Object.assign(params, { platformCode: 'WH' })
