  ...
  <basic-container>
    ...
    >
      ...
      <el-table-column slot="result" label="结果">
        <template v-slot="{row}">
          <g-text-ellipsis v-if="row.status==='失败'" lines="3">
      ...
      <el-table-column slot="operation" label="操作">
        <template v-slot="{row}">
          ...
          <g-auth code="prs_importOrExport_delete">
            <auto-button v-if="row.status!=='已删除'" auto-confirm @confirm="delBill(row)">
              删除
            </auto-button>
          </g-auth>
        </template>
...
import { getFileTask, getPlatformListUnlimit, getStoreCodeListUnlimit, delFileTask, getTaskTypeList } from '@/api/common'
