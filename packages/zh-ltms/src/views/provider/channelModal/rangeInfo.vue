      ...
      <el-col :span="8">
        <el-form-item prop="startShipment" label="启运地：" label-width="150px">
          <commonCascader v-model="rangeInfo.startShipment"
                          ...
                          name="startShipment"
                          @init="newVal =>{changeStartShipment(newVal)}"
          />
        </el-form-item>
      </el-col>
      <el-col :span="1" />
...
export default {
  ...
  data() {
    return {
      ...
      oldShipment: [], // 存储启运地旧数据
      disabled: true,
      flag: [false, false], // 是否有修改值
  ...
  methods: {
    changeStartShipment(val) {
      this.startShipmentMiddleList = val
      // 比较是否有更改
      const newValue = val && val.map(item => item.slice(item.length - 1)[0])
      this.flag[0] = JSON.stringify(newValue) !== JSON.stringify(this.oldShipment)
      this.disabled = !this.flag.some(item => item)
    },
    changeCountry(val) {
      this.flag[1] = JSON.stringify(this.rangeInfo.countryDTOList.sort()) !== JSON.stringify(this.oldCountry.sort())
      this.disabled = !this.flag.some(item => item)
    },
    getLabel(item) {
      ...
      Promise.all([
        ...
        this.detailInfo.channelCode && getServiceScope(
          ...
          if (data) {
            ...
            // 存储比较值
            this.oldShipment = data.channelLimitStartShipmentVOList.map(item => item.startShipmentCode)
            this.oldCountry = this.rangeInfo.countryDTOList
          }
        })
      }).catch(() => {})
    },
    ...
    copyToAdd(val, label, allList, labelList) {
      ...
      if (val) {
        ...
        const allCopyId = allCopyArray.map(item => item.countryNameShort)
