      ...
      <el-col v-if="valueInfo.insuranceService===1" :span="8">
        <el-form-item prop="insurancePrice" label="最高赔付金额: ">
          <el-input v-model="valueInfo.insurancePrice"
                    ...
                    :disabled="isDetail"
                    @input=" newVal => {
                      valueInfo.insurancePrice= limitDecimal(newVal,3)
                    }"
          />
        </el-form-item>
      </el-col>
      ...
      <el-col v-if="valueInfo.insuranceType===1" :span="8">
        <el-form-item prop="insuranceRate" label="赔付比率: ">
          <el-input v-model="valueInfo.insuranceRate"
                    ...
                    :disabled="isDetail"
                    maxlength="13"
                    @input="newVal=>{valueInfo.insuranceRate= limitDecimal(newVal,6)}"
          />
        </el-form-item>
      </el-col>
    ...
    getData() {
      ...
      Promise.all([
        ...
        if (res[2].data) {
          ...
          this.valueInfo.insurancePrice = insurancePrice !== null
            ? Number(Number(insurancePrice).toFixed(6))
            : ''
          this.valueInfo.insuranceRate = insuranceRate !== null
            ? Number(Number(insuranceRate).toFixed(6))
            : ''
        }
      })
    },
