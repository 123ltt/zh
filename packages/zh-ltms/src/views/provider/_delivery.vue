      ...
      <template slot="header">
        <el-form ref="formRef" :model="formData" size="mini" label-width="100px" class="no-message form-col">
          <el-row :gutter="10">
            ...
            <el-col :span="6">
              <el-form-item label="分销渠道" prop="type">
                <el-select v-model="formData.type" clearable placeholder="请选择" filterable class="w-85">
                  <el-option v-for="item in checkList" :key="item.dictKey" :label="item.dictValue" :value="item.dictKey" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="启用状态" prop="status">
                <el-radio-group v-model="formData.status" @change="search">
                  <el-radio-button
            ...
            <el-col :span="6" class="d-flex justify-content-start pb-2 ">
              ...
              <el-button type="primary" class="ms-3" size="mini" icon="el-icon-plus" plain @click="addpoPup()">新增</el-button>
            </el-col>
          </el-row>
        </el-form>
        <!-- 表体 -->
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { prop: 'type', label: '分销渠道', minWidth: '90px', align: 'center' },
  ...
  methods: {
    ...
    init() {
      this.$refs.table.load(this.current, this.formData)
    },
