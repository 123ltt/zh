  ...
  methods: {
    ...
    initProperty(val) {
      Array.isArray(val) && val.forEach((item, i, arr) => {
        if (item.parentId === 0) {
          this.searchParentId(arr, item[this.options.searchParentId])
        }
      })
      return this.result
    },
    searchParentId(list, key, newArray = []) {
      const newList = list.filter(item => item.parentId + '' === key + '')
      !newList[0] ? !newArray[0]
        ? this.result.push([key])
        : this.result.push(Array.from(new Set([...newArray]))) : newList.forEach(item => {
        const array = [...newArray, key, item[this.options.searchParentId]]
        this.searchParentId(list, item[this.options.searchParentId], array)
      })
    },
