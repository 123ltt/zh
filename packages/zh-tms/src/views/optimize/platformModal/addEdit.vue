        ...
        <el-row :gutter="10">
          <el-col :span="8">
            <el-form-item prop="weightLow" label="重量: ">
              <el-input v-model="platformLimitInfo.weightLow" v-input.number.positive maxlength="13" class="small" @input="controlPoint('weightLow')" />~
              <el-input v-model="platformLimitInfo.weightHigh" v-input.number.positive maxlength="13" class="small" @input="controlPoint('weightHigh')" />(单位：kg)
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item prop="totalPriceLow" label="总金额: ">
              <el-input v-model="platformLimitInfo.totalPriceLow" v-input.number.positive maxlength="13" class="small" @input="controlPoint('totalPriceLow')" />~
              <el-input v-model="platformLimitInfo.totalPriceHigh" v-input.number.positive maxlength="13" class="small" @input="controlPoint('totalPriceHigh')" />(单位：美元)
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item prop="priceLow" label="单件金额: ">
              <el-input v-model="platformLimitInfo.priceLow" v-input.number.positive maxlength="13" class="small" @input="controlPoint('priceLow')" />~
              <el-input v-model="platformLimitInfo.priceHigh" v-input.number.positive maxlength="13" class="small" @input="controlPoint('priceHigh')" />(单位：美元)
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="10">
          <el-col :span="8">
            <el-form-item prop="orderFreightLow" label="订单运费: ">
              <el-input v-model="platformLimitInfo.orderFreightLow" v-input.number.positive maxlength="13" class="small" @input="controlPoint('orderFreightLow')" />~
              <el-input v-model="platformLimitInfo.orderFreightHigh" v-input.number.positive maxlength="13" class="small" @input="controlPoint('orderFreightHigh')" />(单位：美元)
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item prop="priority" label="优先级: ">
              <el-input v-model="platformLimitInfo.priority" v-input.number.positive.numeric clearable @blur="blurCheck" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item prop="onlineChannel" label="线上渠道: ">
              <el-radio-group v-model="platformLimitInfo.onlineChannel" class="w-100">
                <el-radio v-for="item in onlineChannelList" :key="item.dictKey" :label="item.dictKey" style="width:18%">{{ item.dictValue }}</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    controlPoint(data) {
      const val = this.platformLimitInfo[data].replace(/[^(0-9|.)]+/, '')
      const newVal = val.match(/^\d+(\.?\d{0,3})/g)
      this.platformLimitInfo[data] = newVal ? newVal[0] : val
    },

    limitSpaceOrGtLt(val) {
      const trimResult = val.trim()
      const result = trimResult.match(/[<>]/g)
      this.platformLimitInfo.remark = result ? val.substring(0, val.length - 1) : trimResult
    },
