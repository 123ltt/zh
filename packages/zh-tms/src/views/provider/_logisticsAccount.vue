            ...
            <el-col :span="12">
              <el-form-item label="状态：" prop="auditStatus">
                <el-radio-group v-model="searchData.auditStatus" @change="changeFunc">
                  <el-radio-button v-for="item in ['全部','已绑定','待绑定']" :key="item" :label="item">{{ item }}</el-radio-button>
                </el-radio-group>
              </el-form-item>
            </el-col>
...
<script>
import { getlogisticsAccountList } from '@/api/logisticsAccount'// 导入表格列表数据接口+账户绑定接口
import { getplatformAllList } from '@/api/platformLimits' // 导入平台列表数据接口
import { getAllTenantPlatformStore } from '@/api/common'// 导入 租户店铺接口
import Accountbinding from '../provider/logisticsAccountModal/Accountbinding'// 导入 绑定组件
export default {
  ...
  methods: {
    ...
    changeFunc() {
      if (this.searchData.auditStatus === '全部') {
        this.searchData.status = ''
        this.initialization()
      } else if (this.searchData.auditStatus === '已绑定') {
