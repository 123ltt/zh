      ...
      >
        <el-table-column label="国家" prop="countryCode" align="center">
          <template v-slot="scope">
            <g-select v-model="scope.row.countryCode"
                      :items="countryList"
        ...
        <el-table-column label="属性" width="300px" align="center">
...
export default {
  ...
  methods: {
    ...
    addColumn() {
      ...
      const emptyCountryIndex = this.formData.detailDTOS
        .findIndex(item => !item.countryCode || !item.property)
      if (emptyCountryIndex !== -1) {
        return this.$message.error(`注意：第${emptyCountryIndex + 1}行的信息为必填选项，请保证数据完整后添加下一条！`)
      }
