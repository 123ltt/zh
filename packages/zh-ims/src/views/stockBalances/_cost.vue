<template>
  ...
  <basic-container>
    <g-table ref="table" :headers="headers" :fetch="fetch" row-key="id">
      ...
      <!-- 货权 -->
      <el-table-column slot="productAuthName" label="货权" width="100" align="center">
        <template v-slot="{row}">
          <g-text-ellipsis lines="2">
            {{ row.productAuthName }}
          </g-text-ellipsis>
      ...
      <!-- 货主 -->
      <el-table-column slot="productOwnerName" label="货主" width="100" align="center">
        <template v-slot="{row}">
          <g-text-ellipsis lines="2">
      ...
      <!-- 仓库归属 -->
      <el-table-column slot="warehouseBelong" label="仓库归属" width="70" align="center">
        <template v-slot="{row}">
...
<script>
import { getList } from '@/api/stockBalances/cost.js'
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {
          label: '期初库存成本',
          prop: 'openingStockQtyCost',
          minWidth: '70px',
          align: 'center'
        },
        {
          label: '期末库存成本',
          prop: 'endingStockQtyCost',
          minWidth: '70px',
          align: 'center'
        },
        {
          label: '增加库存成本',
          prop: 'addStockQtyCost',
          minWidth: '70px',
          align: 'center'
        },
        {
  ...
  methods: {
    ...
    detailBtn(val) {
      this.$newTab({
        path: '/stockBalances/cost/detail',
        title: '库存成本详情',
