<template>
  ...
  <basic-container v-loading="loading">
        ...
        <el-row :gutter="30">
          ...
          <el-col :span="8">
            <el-form-item label="上架单号">
              <el-input :value="detailData.upShelfCode" disabled />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="上架日期">
              <el-input :value="detailData.upShelfDate" disabled />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="上架数量">
              <el-input :value="detailData.upShelfQty" disabled />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <el-card>
      <div slot="header">
        <span style="font-weight: 600;">上架明细</span>
      </div>
      ...
      <el-table :data="recordData" max-height="400" stripe style="width:100%" size="mini" border class="g-table">
        ...
        <el-table-column prop="upShelfQty" label="上架数量" align="center" min-width="90">
          <template slot-scope="scope">
            <p v-for="(item,index) in scope.row.stockLocationList" :key="index" style="margin:0">{{ item.upShelfQty }}</p>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
...
<script>
import { getDetail } from '@/api/inWarehouse/onShelves.js'
export default {
  name: 'OnShelvesDetail',
  props: {
    id: String,
  ...
  methods: {
    getData(id) {
      ...
      getDetail(id).then(res => {
        ...
        this.recordData = res.data.upShelfDetailList
        this.submitData = res.data.upShelfDetailList
        this.loading = false
      })
    },
    submit() {
